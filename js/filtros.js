...
bodega.addEventListener('change', () => {
    if(bodega.value === 'Todas las Bodegas' && variedad.value === 'Todas las Variedades'){
        generales.innerHTML='';
        ...
        variedades(prod);
    }else if(bodega.value === 'Todas las Bodegas' && variedad.value != 'Todas las Variedades'){
        generales.innerHTML='';
        ...
        generales.innerHTML='';
        filtro = []
        let prodVariedad = prod.filter(producto => producto.bodega === bodega.value)
        ...
        generales.innerHTML='';
        let prodVariedad = filtro.filter(producto => producto.bodega === bodega.value)
        prodVariedad.forEach((producto) => {
            cargaDomProductoGenerales(producto);
        });
        //Imprimo las etiquetas en los Botones si estan en el carrito
...
variedad.addEventListener('change', () => {
    if(bodega.value === 'Todas las Bodegas' && variedad.value === 'Todas las Variedades'){
        ...
        generales.innerHTML='';
        filtro = []
        let prodVariedad = prod.filter(producto => producto.variedad === variedad.value)
        prodVariedad.forEach((producto) => {
            cargaDomProductoGenerales(producto);
            filtro.push(producto)
        });
        ...
        let prodVariedad = filtro.filter(producto => producto.variedad === variedad.value)
        generales.innerHTML='';
        prodVariedad.forEach((producto) => {
            cargaDomProductoGenerales(producto);
        });
